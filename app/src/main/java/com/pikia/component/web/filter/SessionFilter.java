...
public class SessionFilter implements Filter {
	...
	public void init(FilterConfig filterConfig) throws ServletException {
		...
		String ignoreUrlsString = this.filterConfig.getInitParameter(IGNORE_URLS);
		if (StringUtils.hasText(ignoreUrlsString)) {
			if (logger.isInfoEnabled()) {
				logger.info("Ignore Urls: " + ignoreUrlsString);
			}
			ignoreUrls = StringUtils.commaDelimitedListToStringArray(ignoreUrlsString);
		} else {
			if (logger.isInfoEnabled()) {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain)
		...
		if (ignoreRequest(httpRequest)) {
			...
			SystemUserDomain user = sessionService.getCurrentUser(httpRequest, SystemUserDomain.class);
			if (user == null) {
				if (isAjax(httpRequest)) {
					System.out.println("--------isAjax(httpRequest)-------------");
					redirect("/d/page/res.ajaxSessionOut", httpRequest,
							(HttpServletResponse) response);
				} else {
					String requestUrl = httpRequest.getRequestURL().toString();
					System.out
							.println("-------- not isAjax(httpRequest)-------------" + requestUrl);
					// redirect("/d/page/login", httpRequest,
					// (HttpServletResponse)response);
					request.getRequestDispatcher("/d/page/login").forward(request, response);
				}
			} else {
	...
	protected boolean ignoreRequest(HttpServletRequest httpRequest) {
		...
		for (String ignoreUrl : ignoreUrls) {
			if (requestUrl.indexOf(context + ignoreUrl) > -1) {
				if (logger.isDebugEnabled()) {
					logger.debug("Matched ignore url: " + ignoreUrl + ", passing......");
				}
				return true;
			}
		}
