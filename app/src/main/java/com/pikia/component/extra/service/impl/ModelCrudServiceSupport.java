package com.pikia.component.extra.service.impl;
...
import com.pikia.component.extra.repository.MyBatisRepository;
...
public abstract class ModelCrudServiceSupport implements ModelCrudService {
	...
	return re.queryForPagination(startIndex, pageSize, queryContext.getSortField(),
		queryContext.getSortType(), params);
    }

    @Transactional(propagation = Propagation.REQUIRED, readOnly = true, rollbackFor = { Exception.class })
    public SortPagedList<?> list(PaginationQueryContext queryContext, Class<?> modelClass) {
	return list(queryContext, modelClass, null, null);
    }

    @Transactional(propagation = Propagation.REQUIRED, readOnly = true, rollbackFor = { Exception.class })
    public SortPagedList<?> list(PaginationQueryContext queryContext, Class<?> modelClass,
	    List pageModelIds, Integer totalCount) {
	...
	pl.orderField(queryContext.getSortField())
		...
		.pageSize(pageSize)
		.totalItemCount(
			totalCount == null ? getTotalCount(queryContext) : totalCount.intValue())
		.setItems(pagedModels);
